...
const editor = reactive({
    ...
    markdown: detail.value.mdContent,
...
watch(
    ...
    () => {
        ...
        editor.markdown = store.view.detail.mdContent
...
const onUploadImage = (event, insertImage, files) => {
    ...
    formdata.append('noteId', store.view.detail.id)
...
const onSave = isCloseEditMode => {
    ...
    ServeEditArticle({
        userId: userId,
        noteId: data.id,
        categoryId: data.class_id,
        title: editor.title,
        mdContent: editor.markdown,
        content: editor.html,
    })
        .then(res => {
            if (res.code != 200) {
                return $message.info(res.data || res.message)
            }
            ...
            store.view.detail.mdContent = editor.markdown
...
const onTitle = e => {
    ...
    // 标题最大字符数量不能超过32个
    if (editor.title.length > 32) {
        $message.info('标题长度不能超过32哦!')
    } else {
...
// 收藏笔记
const onCollection = () => {
    let type = detail.value.stared == true ? 2 : 1
...
const onDownload = () => {
    ...
    let blob = new Blob([store.view.detail.mdContent], {
        type: 'text/plain',
    })
    ...
    <section class="el-container section" :class="{ full: isFull }" v-else>
        <main class="el-main" style="padding: 0 5px">
            <section class="el-container is-vertical height100">
                ...
                >
                    <span>分类: {{ store.view.detail.categoryName || '默认分类' }}</span>
                    <span>最后更新于 {{ store.view.detail.updateTime }}</span>
                </header>
        ...
        <aside class="el-aside nav-tools">
            ...
            <div
                v-show="detail.id"
                class="nav-item"
                :class="{ active: detail.stared == true }"
                @click="onCollection"
            >
